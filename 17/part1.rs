...
fn main() {
    ...
    for (y, line) in include_str!("input.txt").lines().enumerate() {
        for (x, chr) in line.chars().enumerate() {
            if chr == '#' {
                grid.grid[INPUT_OFFSET][y + INPUT_OFFSET][x + INPUT_OFFSET] = true;
            }
        }
    }
    ...
    for x in 0..GRID_SIZE {
        for y in 0..GRID_SIZE {
            for z in 0..GRID_SIZE {
                let active = curr_grid.get(x as isize, y as isize, z as isize);
                let n = neighbours(x as isize, y as isize, z as isize)
                    .filter(|&(x, y, z)| curr_grid.get(x, y, z))
                    .count();
                ...
                next_grid.grid[z][y][x] = result;
            }
        }
    }
}

fn neighbours(x: isize, y: isize, z: isize)
